...
const TrailerCard = ({trailer,streamingKey,setStreamingKey}:trailerCard) => {
    ...
    return (
        <div className={`bg-indigo-300 bg-opacity-60 my-1 py-1 px-2 cursor-pointer ${trailer.key ===streamingKey ? "bg-sky-700 text-cyan-300":""}`} onClick={()=>setStreamingKey(trailer.key)}>
            <h2>{trailer.name}</h2>
        </div>
    );
};
